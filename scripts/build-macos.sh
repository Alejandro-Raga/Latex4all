...
echo "==> Building Latex4All $TAG for macOS ($TARGET)"
...
pnpm --filter @latex4all/desktop tauri build --target "$TARGET"

# Notarize DMG
DMG_PATH=$(find "apps/desktop/src-tauri/target/$TARGET/release/bundle/dmg" -name '*.dmg' | head -1)
APP_PATH="apps/desktop/src-tauri/target/$TARGET/release/bundle/macos/Latex4All.app"
  ...
  if [ -f "$LATEST_JSON" ]; then
    ...
    node -e "
      ...
      data.platforms['darwin-aarch64'] = {
        signature: \`$SIGNATURE\`,
        url: 'https://github.com/delibae/claude-prism/releases/download/$TAG/Latex4All-macOS.app.tar.gz'
      };
...
gh release view "$TAG" --repo delibae/claude-prism >/dev/null 2>&1 || \
  gh release create "$TAG" --repo delibae/claude-prism --title "Latex4All $TAG" --generate-notes

# Rename to version-free names
RENAMED_DMG="apps/desktop/src-tauri/target/Latex4All-macOS.dmg"
cp "$DMG_PATH" "$RENAMED_DMG"
UPLOAD_ASSETS=("$RENAMED_DMG")

if [ -n "${UPDATE_TAR:-}" ]; then
  RENAMED_TAR="apps/desktop/src-tauri/target/Latex4All-macOS.app.tar.gz"
