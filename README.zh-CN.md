...
<p align="center">
  <img src="./assets/demo.png" alt="Latex4All 演示" width="800" />
</p>
...
Latex4All 是**本地优先**的替代方案 — 文件存储在本地磁盘，离线编译。AI 功能通过 Anthropic API 发送内容进行推理（参见[数据使用政策](https://code.claude.com/docs/en/data-usage)）。
...
### Python 环境（uv）
Latex4All 集成了 [uv](https://docs.astral.sh/uv/) — 快速的 Python 包管理器。一键安装 uv，一键创建项目级虚拟环境。Claude Code 在运行 Python 代码时自动使用 `.venv`，因此您可以在编辑器中直接生成图表、运行分析脚本和处理数据。
...
### 模板与项目向导快速开始
选择模板（论文、学位论文、演示文稿、海报、信函等），命名，可选描述写作内容 — Latex4All 设置项目并通过 AI 生成初始内容。拖放参考文件（PDF、BIB、图片）即可立即开始写作。
...
### 基于 Git 的历史记录
每次保存都会在本地 Git 仓库（`.latex4all/history.git/`）中创建快照。标记重要检查点，浏览任意两个快照之间的差异，恢复以前的版本 — 全部在应用内完成。
...
从 [GitHub Releases](https://github.com/Alejandro-Raga/Latex4all/releases) 下载最新版本。
